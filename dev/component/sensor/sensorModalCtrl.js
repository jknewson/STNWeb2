(function () {
    ...
    ModalControllers.controller('fullSensorModalCtrl', ['$scope', '$filter', '$timeout', '$cookies', '$http', '$uibModalInstance', '$uibModal', 'allDepDropdowns', 'allStatusTypes', 'allInstCollCond', 'allEvents', 'allDepTypes', 'thisSensor', 'SensorSite', 'siteOPs', 'allMembers', 'INSTRUMENT', 'INSTRUMENT_STATUS', 
        function fullSensorModalCtrl($scope, $filter, $timeout, $cookies, $http, $uibModalInstance, $uibModal, allDepDropdowns, allStatusTypes, allInstCollCond, allEvents, allDepTypes, thisSensor, SensorSite, siteOPs, allMembers, INSTRUMENT, INSTRUMENT_STATUS) {
            ...
            if ($scope.RetrievedSensorStat.STATUS_TYPE_ID === 2) $scope.statusTypeList = allStatusTypes.filter(function (s) { return s.STATUS == 'Lost'; });
            if ($scope.RetrievedSensorStat.STATUS_TYPE_ID === 3) $scope.statusTypeList = allStatusTypes.filter(function (s) { return s.STATUS == 'Retrieved'; });
            ...
            $scope.saveDeployed = function (valid, sensorToUpdate) {
                if (valid) {
                    var updatedSensor = {}; var updatedSenStat = {};
                    //see if they used Minutes or seconds for interval. need to store in seconds
                    if ($scope.IntervalType.type == "Minutes")
            ...
            $scope.saveRetrieved = function (valid, sensorToUpdate) {
                if (valid) {
                    var updatedSensor = {}; var updatedSenStat = {};
                    //see if they used Minutes or seconds for interval. need to store in seconds
                    if ($scope.IntervalType.type == "Minutes")
            ...
            $scope.ventOptions = [
                { value: 1, text: 'Yes' },
                { value: 2, text: 'No' }];
            //show the vented options for xeditable
            $scope.showVentedOptions = function () {
                var selected = $filter('filter')($scope.ventOptions, { text: $scope.sensor.VENTED });
                return ($scope.sensor.VENTED && selected.length) ? selected[0] : '';
            };

            $scope.IntervalType = 'Seconds'; //default of what it is stored in

            $scope.intervalOptions = [
                { value: 1, text: 'Seconds' },
                { value: 2, text: 'Minutes' }];
            //show the vented options for xeditable
            $scope.showIntervalOptions = function () {
                var selected = $filter('filter')($scope.intervalOptions, { text: $scope.IntervalType });
                return ($scope.IntervalType && selected.length) ? selected[0].text : '';
            };
